...
static ITNode *it_new_node(
    int start, int end, int session_id,
...
static ITNode *it_insert(
    ITNode *node, int start, int end,
...
static ITNode *it_find_overlap(ITNode *node, int s, int e, const char *date) {
    ...
    if (strcmp(node->date, date) == 0 && node->start < e && s < node->end)
        return node;
...
static void load_interval_tree(void) {
    ...
    if (mysql_query(conn,
        ...
        "FROM sessions WHERE session_date IS NOT NULL"))
        return;
...
static void extract_real_ip(
    const char *req, const char *socket_ip,
...
static void get_current_date(char *out) {
    time_t now = time(NULL);
    struct tm *lt = localtime(&now);
    strftime(out, 12, "%Y-%m-%d", lt);
}

void init_database(void) {
    conn = mysql_init(NULL);
    if (!mysql_real_connect(
            conn, "localhost", "root", "12345", "attendance_db", 0, NULL, 0)) {
...
static void api_register_student(SOCKET client, char *body) {
    ...
    if (p) {
        p += 5;
        char *e = strstr(p, "&");
        if (e) { int l = (int)(e - p); if (l > 0 && l < 100) strncpy(raw_name, p, l); }
        else strncpy(raw_name, p, 99);
    }
    p = strstr(body, "email=");
    if (p) {
        p += 6;
        char *e = strstr(p, "&");
        if (e) { int l = (int)(e - p); if (l > 0 && l < 100) strncpy(raw_email, p, l); }
        else strncpy(raw_email, p, 99);
    }
    ...
    if (!strlen(prn) || !strlen(name) || !strlen(email)) {
    ...
    sprintf(query,
        ...
        safe_prn, safe_name, safe_email);

    if (mysql_query(conn, query)) {
        if (mysql_errno(conn) == 1062) {
            send_response(client, "application/json",
                "{\"success\":false,\"message\":\"A student with this PRN already exists.\"}");
        } else {
            char err[300];
            sprintf(err, "{\"success\":false,\"message\":\"Database error: %s\"}",
                mysql_error(conn));
...
static void api_register_students_csv(SOCKET client, char *body) {
    ...
    if (!decoded) {
        send_response(client, "application/json",
            "{\"success\":false,\"message\":\"Out of memory\"}");
        return;
    }
    ...
    while (line) {
        char raw_prn[50] = {0}, raw_name[100] = {0}, raw_email[100] = {0};
        if (portable_strncasecmp(line, "prn", 3) == 0 ||
            portable_strncasecmp(line, "\"prn", 4) == 0) {
            line = strtok(NULL, "\r\n");
            continue;
        }
    ...
    char resp[256];
    sprintf(resp,
        "{\"success\":true,\"message\":\"%d student(s) added, "
        "%d skipped (duplicates or invalid rows).\"}",
        added, skipped);
...
static void api_get_students(SOCKET client) {
    ...
    MYSQL_RES *res = mysql_store_result(conn);
    if (!res) { send_response(client, "application/json", "[]"); return; }

    char *json = (char *)malloc(MAX_JSON);
    if (!json) { mysql_free_result(res); send_response(client, "application/json", "[]"); return; }
    ...
    while ((row = mysql_fetch_row(res))) {
        json_escape(ep, sizeof(ep), row[0] ? row[0] : "");
        json_escape(en, sizeof(en), row[1] ? row[1] : "");
        json_escape(ee, sizeof(ee), row[2] ? row[2] : "");
        char item[700];
...
static void api_create_session(SOCKET client, char *body) {
    ...
    p = strstr(body, "teacher=");
    if (p) {
        p += 8;
        char *e = strstr(p, "&");
        if (e) { int l = (int)(e - p); if (l > 0 && l < 100) strncpy(raw_teacher, p, l); }
        else strncpy(raw_teacher, p, 99);
    }
    p = strstr(body, "session=");
    if (p) {
        p += 8;
        char *e = strstr(p, "&");
        if (e) { int l = (int)(e - p); if (l > 0 && l < 100) strncpy(raw_session, p, l); }
        else strncpy(raw_session, p, 99);
    }
    p = strstr(body, "date=");
    if (p) {
        p += 5;
        char *e = strstr(p, "&");
        if (e) { int l = (int)(e - p); if (l > 0 && l < 20) strncpy(raw_date, p, l); }
        else strncpy(raw_date, p, 19);
    }
    p = strstr(body, "start=");
    if (p) {
        p += 6;
        char *e = strstr(p, "&");
        if (e) { char t[20] = {0}; int l = (int)(e - p); if (l > 0 && l < 20) strncpy(t, p, l); start = atoi(t); }
        else start = atoi(p);
    }
    p = strstr(body, "end=");
    if (p) {
        p += 4;
        char *e = strstr(p, "&");
        if (e) { char t[20] = {0}; int l = (int)(e - p); if (l > 0 && l < 20) strncpy(t, p, l); end = atoi(t); }
        else end = atoi(p);
    }

    url_decode(teacher, raw_teacher, sizeof(teacher));
    url_decode(sess,    raw_session, sizeof(sess));
    url_decode(date,    raw_date,    sizeof(date));

    if (!strlen(teacher) || !strlen(sess) || end <= start) {
        send_response(client, "application/json",
            "{\"success\":false,\"message\":"
            "\"All fields are required and end time must be after start time.\"}");
        return;
    }
    ...
    if (strlen(date) >= 10) {
        ...
        if (ov) {
            char msg[300], ts[10], te[10];
            minutes_to_time(ov->start, ts);
            minutes_to_time(ov->end, te);
            snprintf(msg, sizeof(msg),
                "{\"success\":false,\"message\":"
                "\"Time overlap with existing session '%s' (%s\xe2\x80\x93%s). "
    ...
    db_escape(safe_date,    date,    sizeof(safe_date));

    char query[800];
    if (strlen(date) >= 10) {
        sprintf(query,
            "INSERT INTO sessions(teacher_name,session_name,session_date,start_time,end_time) "
            "VALUES('%s','%s','%s',%d,%d)",
            safe_teacher, safe_session, safe_date, start, end);
    } else {
        sprintf(query,
            "INSERT INTO sessions(teacher_name,session_name,start_time,end_time) "
            "VALUES('%s','%s',%d,%d)",
            safe_teacher, safe_session, start, end);
    }
...
static void api_get_sessions(SOCKET client) {
    ...
    while ((row = mysql_fetch_row(res))) {
        ...
        int st = row[4] ? atoi(row[4]) : 0;
        int en = row[5] ? atoi(row[5]) : 0;
        minutes_to_time(st, start_str);
        minutes_to_time(en, end_str);
        json_escape(et, sizeof(et), row[1] ? row[1] : "");
        json_escape(es, sizeof(es), row[2] ? row[2] : "");
        const char *date_val =
            (row[3] && strcmp(row[3], "0000-00-00") != 0) ? row[3] : "";
        char item[600];
        int n = snprintf(item, sizeof(item),
            ...
            first ? "" : ",",
            row[0] ? row[0] : "0",
            et, es, date_val, start_str, end_str);
        if (used + n + 2 < MAX_JSON) {
            strcat(json, item);
            used += n;
            first = 0;
...
/* ─────────────────────────────────────────────────────────────────────────────
   api_mark_attendance
   FIX: Before inserting the attendance record, verify that the student has
        face_registered = 1.  If not, reject with a clear error message.
        This is a server-side enforcement that works even if the frontend check
        is somehow bypassed (e.g., direct HTTP request).
───────────────────────────────────────────────────────────────────────────── */
static void api_mark_attendance(SOCKET client, char *body, const char *client_ip) {
    ...
    sscanf(body, "prn=%49[^&]&name=%99[^&]&session_id=%d",
        raw_prn, raw_name, &session_id);
    url_decode(prn,  raw_prn,  sizeof(prn));
    url_decode(name, raw_name, sizeof(name));

    if (session_id <= 0 || !strlen(prn)) {
        send_response(client, "application/json",
            "{\"success\":false,\"message\":"
            "\"Invalid input. Please go back and try again.\"}");
        return;
    }

    /* ── SERVER-SIDE FACE REGISTRATION CHECK ─────────────────────────────── */
    db_escape(safe_prn, prn, sizeof(safe_prn));

    char face_check_query[256];
    sprintf(face_check_query,
        "SELECT face_registered FROM students WHERE prn='%s'",
        safe_prn);

    if (mysql_query(conn, face_check_query)) {
        send_response(client, "application/json",
            "{\"success\":false,\"message\":\"Database error. Please try again.\"}");
        return;
    }

    MYSQL_RES *face_res = mysql_store_result(conn);
    if (!face_res) {
        send_response(client, "application/json",
            "{\"success\":false,\"message\":\"Database error. Please try again.\"}");
        return;
    ...
    MYSQL_ROW face_row = mysql_fetch_row(face_res);
    if (!face_row) {
        mysql_free_result(face_res);
        send_response(client, "application/json",
            "{\"success\":false,\"message\":"
            "\"Student not found. Please check your PRN.\"}");
        return;
    }

    int face_registered = atoi(face_row[0] ? face_row[0] : "0");
    mysql_free_result(face_res);

    if (!face_registered) {
        send_response(client, "application/json",
            "{\"success\":false,\"message\":"
            "\"Face not registered. You must register your face before marking attendance. "
            "Please ask your teacher to send you the face-registration email.\"}");
        return;
    }
    /* ── END FACE REGISTRATION CHECK ─────────────────────────────────────── */

    char sess_query[256];
    sprintf(sess_query,
        "SELECT session_date, start_time, end_time "
        "FROM sessions WHERE id=%d",
        session_id);

    if (mysql_query(conn, sess_query)) {
        send_response(client, "application/json",
            "{\"success\":false,\"message\":\"Database error. Please try again.\"}");
        return;
    }

    MYSQL_RES *sess_res = mysql_store_result(conn);
    if (!sess_res) {
        send_response(client, "application/json",
            "{\"success\":false,\"message\":\"Database error. Please try again.\"}");
        return;
    ...
    MYSQL_ROW sess_row = mysql_fetch_row(sess_res);
    if (!sess_row) {
        mysql_free_result(sess_res);
        send_response(client, "application/json",
            "{\"success\":false,\"message\":\"Session not found.\"}");
        return;
    }

    const char *sess_date  = sess_row[0] ? sess_row[0] : "";
    int sess_start         = sess_row[1] ? atoi(sess_row[1]) : 0;
    int sess_end           = sess_row[2] ? atoi(sess_row[2]) : 0;
    mysql_free_result(sess_res);

    char today[12] = {0};
    get_current_date(today);
    int now_mins = get_current_minutes();

    if (strlen(sess_date) >= 10 && strcmp(sess_date, "0000-00-00") != 0) {
        if (strcmp(today, sess_date) != 0) {
            char msg[256];
            snprintf(msg, sizeof(msg),
                "{\"success\":false,\"message\":"
                "\"Attendance can only be marked on %s. "
                "Today is %s.\"}",
                sess_date, today);
            send_response(client, "application/json", msg);
            return;
        }
        if (now_mins < sess_start) {
            char ts[10], te[10];
            minutes_to_time(sess_start, ts);
            minutes_to_time(sess_end,   te);
            char msg[256];
            snprintf(msg, sizeof(msg),
                "{\"success\":false,\"message\":"
                "\"Session has not started yet. "
                "It is scheduled from %s to %s.\"}",
                ts, te);
            send_response(client, "application/json", msg);
            return;
        }
        if (now_mins >= sess_end) {
            char ts[10], te[10];
            minutes_to_time(sess_start, ts);
            minutes_to_time(sess_end,   te);
            char msg[256];
        ...
        if (mysql_errno(conn) == 1062) {
            ...
            if (strstr(dberr, "unique_prn_session")) {
                ...
                send_response(client, "application/json",
                    ...
                    "Each device may only be used once per session.\"}");
            } else {
                send_response(client, "application/json",
                    "{\"success\":false,\"message\":"
    ...
    send_response(client, "application/json",
        ...
        "\"Attendance marked successfully! You may now close this page.\"}");
}
...
static void api_session_attendance(SOCKET client, const char *query_str) {
    ...
    char query[256];
    sprintf(query,
        "SELECT prn,name,marked_at,device_ip "
        "FROM attendance WHERE session_id=%d ORDER BY marked_at",
        session_id);
    ...
    while ((row = mysql_fetch_row(res))) {
        json_escape(ep, sizeof(ep), row[0] ? row[0] : "");
        json_escape(en, sizeof(en), row[1] ? row[1] : "");
        json_escape(em, sizeof(em), row[2] ? row[2] : "");
        json_escape(ei, sizeof(ei), row[3] ? row[3] : "");
        char item[600];
        int n = snprintf(item, sizeof(item),
            "%s{\"prn\":\"%s\",\"name\":\"%s\","
...
static void api_search_attendance(SOCKET client, const char *query_str) {
    ...
    char query[512];
    sprintf(query,
        "SELECT s.session_name,s.teacher_name,a.marked_at,s.session_date "
        "FROM attendance a "
    ...
    while ((row = mysql_fetch_row(res))) {
        json_escape(es, sizeof(es), row[0] ? row[0] : "");
        json_escape(et, sizeof(et), row[1] ? row[1] : "");
        json_escape(em, sizeof(em), row[2] ? row[2] : "");
        json_escape(ed, sizeof(ed), row[3] ? row[3] : "");
        char item[700];
        int n = snprintf(item, sizeof(item),
            "%s{\"session_name\":\"%s\",\"teacher_name\":\"%s\","
            "\"marked_at\":\"%s\",\"session_date\":\"%s\"}",
            first ? "" : ",", es, et, em, ed);
        if (used + n + 2 < MAX_JSON) {
            strcat(json, item);
            used += n;
...
static void api_dashboard_stats(SOCKET client) {
    ...
    if (!mysql_query(conn, "SELECT COUNT(*) FROM students")) {
        ...
        if (res && (row = mysql_fetch_row(res))) ts = atoi(row[0] ? row[0] : "0");
        if (res) mysql_free_result(res);
    }
    if (!mysql_query(conn, "SELECT COUNT(*) FROM sessions")) {
        res = mysql_store_result(conn);
        if (res && (row = mysql_fetch_row(res))) tse = atoi(row[0] ? row[0] : "0");
        if (res) mysql_free_result(res);
    }
    if (!mysql_query(conn, "SELECT COUNT(*) FROM attendance")) {
        res = mysql_store_result(conn);
        if (res && (row = mysql_fetch_row(res))) ta = atoi(row[0] ? row[0] : "0");
        if (res) mysql_free_result(res);
    }

    char resp[128];
    sprintf(resp,
        "{\"total_students\":%d,\"total_sessions\":%d,\"total_attendance\":%d}",
        ts, tse, ta);
    send_response(client, "application/json", resp);
}
...
static void api_delete_student(SOCKET client, char *body) {
    ...
    if (mysql_query(conn, q2)) {
        send_response(client, "application/json",
            "{\"success\":false,\"message\":\"Database error. Could not delete student.\"}");
        return;
    }
    send_response(client, "application/json",
        "{\"success\":true,\"message\":\"Student and all attendance records deleted.\"}");
}

static void api_delete_attendance(SOCKET client, char *body) {
    int sid = 0;
    sscanf(body, "session_id=%d", &sid);
    if (sid <= 0) {
        send_response(client, "application/json",
    ...
    char q[128];
    sprintf(q, "DELETE FROM attendance WHERE session_id=%d", sid);
    ...
    send_response(client, "application/json",
        "{\"success\":true,\"message\":"
        "\"All attendance records for this session have been cleared.\"}");
}
...
static void api_delete_session(SOCKET client, char *body) {
    ...
    send_response(client, "application/json",
        "{\"success\":true,\"message\":"
        "\"Session and all its attendance records deleted.\"}");
}
...
static void api_remove_attendance_record(SOCKET client, char *body) {
    ...
    char q[256];
    sprintf(q,
        "DELETE FROM attendance WHERE session_id=%d AND prn='%s'",
        sid, safe_prn);
    if (mysql_query(conn, q)) {
        send_response(client, "application/json",
            "{\"success\":false,\"message\":\"Database error.\"}");
    ...
    send_response(client, "application/json",
        "{\"success\":true,\"message\":\"Attendance record removed. Student marked absent.\"}");
}
...
static void api_teacher_mark_attendance(SOCKET client, char *body) {
    ...
    if (mysql_query(conn, name_query)) {
        send_response(client, "application/json",
            "{\"success\":false,\"message\":\"Database error.\"}");
        return;
    }
    ...
    if (!name_row) {
        ...
        send_response(client, "application/json",
            "{\"success\":false,\"message\":\"Student not found. Please check the PRN.\"}");
        return;
    }
    char name[200] = {0};
    ...
    if (mysql_query(conn, query)) {
        if (mysql_errno(conn) == 1062) {
            send_response(client, "application/json",
                "{\"success\":false,\"message\":"
                "\"This student's attendance is already recorded for this session.\"}");
        } else {
            send_response(client, "application/json",
                "{\"success\":false,\"message\":\"Database error.\"}");
        }
    ...
    send_response(client, "application/json",
        "{\"success\":true,\"message\":\"Student marked present.\"}");
}
...
static void api_update_face_status(SOCKET client, char *body) {
    ...
    if (!strlen(prn)) {
        send_response(client, "application/json",
            "{\"success\":false,\"message\":\"PRN required.\"}");
        return;
    }
    db_escape(safe_prn, prn, sizeof(safe_prn));
    ...
    char query[256];
    sprintf(query,
        "UPDATE students SET face_registered=%d WHERE prn='%s'",
        status, safe_prn);
    if (mysql_query(conn, query)) {
        char err[300];
        sprintf(err,
            "{\"success\":false,\"message\":\"Database error: %s\"}",
            mysql_error(conn));
        send_response(client, "application/json", err);
        return;
    }
...
static void api_verify_face(SOCKET client, char *body) {
    ...
    if (!strlen(prn)) {
        send_response(client, "application/json",
            "{\"success\":false,\"verified\":false,\"message\":\"PRN required.\"}");
        return;
    }
    db_escape(safe_prn, prn, sizeof(safe_prn));
    ...
    sprintf(chk,
        "SELECT face_registered FROM students WHERE prn='%s'",
        safe_prn);
    if (mysql_query(conn, chk)) {
        send_response(client, "application/json",
            "{\"success\":false,\"verified\":false,\"message\":\"Database error.\"}");
        return;
    }
    MYSQL_RES *res = mysql_store_result(conn);
    if (!res) {
    ...
    if (!face_reg) {
        /* Return face_registered:false so the frontend can show the hard-block message */
        send_response(client, "application/json",
    ...
    if (!img_decoded) {
        send_response(client, "application/json",
            "{\"success\":false,\"verified\":false,\"message\":\"Out of memory.\"}");
        return;
    }
    url_decode(img_decoded, img_start, img_raw_len + 1);
    ...
    if (!f) {
        free(img_decoded);
        send_response(client, "application/json",
            "{\"success\":false,\"verified\":false,\"message\":\"Temp file error.\"}");
        return;
    }
    fputs(img_decoded, f);
    fclose(f);
    ...
    char cmd[512];
    snprintf(cmd, sizeof(cmd),
        "python verify_face.py \"%s\" \"%s\" \"%s\"",
        prn, tmp_b64, result_file);
    system(cmd);
    remove(tmp_b64);

    FILE *rf = fopen(result_file, "r");
    if (!rf) {
        /* verify_face.py did not produce output — treat as unverified, not a bypass */
        send_response(client, "application/json",
            "{\"success\":false,\"verified\":false,\"face_registered\":true,"
    ...
    if (strlen(result_buf) > 0) {
        ...
        send_response(client, "application/json",
            "{\"success\":false,\"verified\":false,\"face_registered\":true,"
            "\"message\":\"Face verification returned an empty result. Please try again.\"}");
    }
}
...
static void api_send_session_emails(SOCKET client, const char *query_str) {
    ...
    char cmd[256];
    snprintf(cmd, sizeof(cmd), "python send_emails.py %d", session_id);
    int ret = system(cmd);
    if (ret == 0) {
        send_response(client, "application/json",
            "{\"success\":true,\"message\":"
            "\"Session emails sent successfully to all students.\"}");
    } else {
        send_response(client, "application/json",
            "{\"success\":false,\"message\":"
...
static void api_send_face_reg_email(SOCKET client, const char *query_str) {
    ...
    p = strstr(query_str, "prn=");
    if (p) {
        p += 4;
        char *e = strstr(p, "&");
        if (e) { int l = (int)(e - p); if (l > 0 && l < 100) strncpy(raw_prn, p, l); }
        else strncpy(raw_prn, p, 99);
    }
    p = strstr(query_str, "&name=");
    if (p) {
        p += 6;
        char *e = strstr(p, "&");
        if (e) { int l = (int)(e - p); if (l > 0 && l < 200) strncpy(raw_name, p, l); }
        else strncpy(raw_name, p, 199);
    }
    p = strstr(query_str, "&email=");
    if (p) {
        p += 7;
        strncpy(raw_email, p, 199);
    }
    ...
    if (!strlen(prn) || !strlen(email)) {
        send_response(client, "application/json",
            "{\"success\":false,\"message\":\"Invalid PRN or email address.\"}");
        return;
    }
    ...
    char cmd[1024];
    snprintf(cmd, sizeof(cmd),
        "python send_face_reg_email.py \"%s\" \"%s\" \"%s\" \"%s\"",
        prn, name, email, server_ip);
    int ret = system(cmd);
    ...
    if (ret == 0) {
        snprintf(resp, sizeof(resp),
            "{\"success\":true,\"message\":"
            "\"Face registration email sent to %s (%s).\"}",
            name, email);
    } else {
        snprintf(resp, sizeof(resp),
            "{\"success\":false,\"message\":"
            "\"Email sending failed for %s (%s). Check SMTP settings.\"}",
            name, email);
    }
    send_response(client, "application/json", resp);
}
...
void handle_request(SOCKET client, char *req, const char *socket_ip) {
    ...
    if (strcmp(path, "/student") == 0 ||
        strcmp(path, "/student.html") == 0) {
        char *html = (char *)malloc(BUFFER_SIZE);
    ...
    if (strcmp(path, "/face-register") == 0 ||
        strcmp(path, "/face-register.html") == 0) {
        char *html = (char *)malloc(BUFFER_SIZE);
        read_file("face-register.html", html);
        send_response(client, "text/html", html);
        free(html);
        return;
    }
